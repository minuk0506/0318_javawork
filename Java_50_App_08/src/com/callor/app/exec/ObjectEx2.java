...
public class ObjectEx2 {
	...
	private static void makeScore(ScoreVO[] scAry) { // main에 makeScore(scVOs) 가 scAry로 연결됨 (주소값만 전달)
		
		for(int i = 0 ; i < scAry.length ; i++) {
			scAry[i] = new ScoreVO();
			scAry[i].setStNum(i + 1);
			scAry[i].setIntKor(getScore(50, 51));
			scAry[i].setIntEng(getScore(50, 51));
			scAry[i].setIntMath(getScore(50, 51));
		}
